...
- Current phase: `Quiz repetition and teacher-authored questions implemented locally; backend deployment blocked`
- Last completed task: Removed literal Firebase client API keys from tracked Dart source, added required per-platform dart-defines with an ignored local configuration, and verified a debug Android build.
- Current task: The updated Android release APK is built and verified locally; the two updated quiz-generation Cloud Functions await production deployment.
- NEXT TASK: Enable billing for Firebase project `studexa-b5e55`, then redeploy `generateQuizHttp` and `generateQuiz` and verify a real production shortfall/continuation. Follow with the pending signed-in DOCX/PPTX Android walkthrough.
- Blockers: Firebase Secret Manager returned HTTP 403 (`billing to be enabled`) during function deployment, before either function was updated. Windows desktop build still requires Windows Developer Mode; iOS needs macOS/Xcode.
- Last verified: 2026-09-20: 23 Node generator tests, 196 Flutter tests, `flutter analyze`, 50 emulator fixture checks for shortfall/manual/one-attempt behavior, and 42 live-Gemini emulator checks passed. `Studexa.apk` built, matched `app-release.apk` by SHA-256, and passed APK signature verification. Production backend is still on the previously deployed version.
...
## COMPLETED TASKS
- [x] Firebase client configuration source hygiene: replaced literal keys in `lib/firebase_options.dart` with required per-platform `String.fromEnvironment` values, added an ignored local define file and tracked placeholder example, documented run/build commands, and retained Security Rules plus console restrictions/App Check as the real authorization boundary.
- [x] Local quiz repetition and teacher-completion implementation: Gemini generation rejects repeated facts across batches and prior draft questions; opted-in incomplete output becomes a private draft; teachers can Use N, request one extra generation attempt, or add a validated question of any of the five types. Legacy exact-count requests remain supported. Production deployment is separately blocked by project billing.
- [x] Shareable APK filename: copied the verified release artifact to `build/app/outputs/flutter-apk/Studexa.apk` without changing the package name, launcher label, icon, signing, or application behavior.
- [x] Connected-phone APK compatibility check: compared the installed Studexa package with the fresh release APK using Android package metadata, AAPT, and APK signer certificates; confirmed the same package ID, version, and signing certificate, with debug build installed and release APK available for an in-place update. No installation or uninstall was performed.
...
## IN PROGRESS
- Deploy the locally verified `generateQuizHttp` and `generateQuiz` changes once Firebase project billing is enabled; new-client shortfall generation depends on these backend versions.
...
## BLOCKED
- 2026-09-20 production function deployment: Firebase Secret Manager returned HTTP 403 requiring billing on `studexa-b5e55`; no function update occurred.
- Launcher home-screen/app-drawer verification is still pending; an Android phone is connected as of 2026-09-20, but no launcher check was performed in this APK compatibility check.
- Shared-project changes were confirmed and deployed; use the updated enrollment and Practice-query client. An Android phone is connected as of 2026-09-20; Windows requires host symlink support and iOS requires macOS/Xcode. Intermittent Gemini 503 responses occurred, but the final real-provider backend check passed.

## FILES CHANGED
- Quiz repetition/teacher completion: `functions/index.js`, `functions/quiz_generator.js`, `functions/test/quiz_generator.test.js`, `functions/test/firebase_integration.js`, `lib/models/quiz_model.dart`, `lib/services/quiz_generation_client.dart`, `lib/services/quiz_service.dart`, `lib/screens/teacher/quiz_detail_screen.dart`, `lib/screens/teacher/upload_generate_quiz_screen.dart`, new `lib/screens/teacher/manual_question_dialog.dart`, `test/quiz_generation_client_test.dart`, `test/ui_navigation_walkthrough_test.dart`, new `test/manual_question_dialog_test.dart`, and this log. Updated ignored artifacts: `build/app/outputs/flutter-apk/app-release.apk` and `Studexa.apk`; `docs/command.md` was pre-existing and untouched.
- Shareable APK filename: created ignored build artifact `build/app/outputs/flutter-apk/Studexa.apk` as an identical copy of `app-release.apk`; updated only `docs/IMPLEMENTATION_LOG.md` among tracked files.
- Connected-phone APK compatibility check: only `docs/IMPLEMENTATION_LOG.md` was updated. No app code, package, signing configuration, installed phone app, or release APK was changed.
...
- 2026-09-20 local quiz schema extension: new `quizzes/{quizId}` drafts store `requestedQuestionCount`, `selectedQuestionTypes`, and `extraGenerationAttempted`; teacher-authored question maps carry `origin: manual`. Existing quizzes remain readable with defaults. Updated `generateQuizHttp`/`generateQuiz` code has NOT been deployed because Secret Manager returned HTTP 403 requiring project billing. No rules or package changed.
...
## TESTS / VERIFICATION
- 2026-09-20 quiz completion pass: `node --test --test-reporter=spec functions/test/quiz_generator.test.js` passed 23/23; `flutter test --reporter compact` passed 196/196; `flutter analyze` found no issues; `git diff --check` found no whitespace errors. Firebase Auth/Firestore emulators with backend fixture passed 50 checks, including private partial draft, retained teacher question and one-attempt limit. Auth/Firestore/Functions emulators with real Gemini passed 42 checks, generating and saving Actual and Practice 10-question quizzes. `flutter build apk --release` succeeded; `Studexa.apk` and `app-release.apk` are both 76,264,773 bytes with SHA-256 `07FCAAED421C788880491CD686F4310805D7BCAA5A1EAAC76E612C365EF14CA9`; `apksigner verify --verbose` passed with one v2 signer. `firebase deploy --only "functions:generateQuizHttp,functions:generateQuiz" --project studexa-b5e55 --non-interactive` failed before upload with Secret Manager HTTP 403 because billing is disabled. No phone installation was performed.
- Shareable APK filename: SHA-256 of `Studexa.apk` equals `app-release.apk` (`8A842EB29F11ADEA64E3D1EBD13C873529E544E77562DAA93C565AC18BCAADE7`); both are 75,953,401 bytes. Date: 2026-09-20.
- Connected-phone APK compatibility check:
  - `adb devices -l` found Android device `584dac09` (CPH1933); `pm path` found installed `com.example.studexa`.
  - Installed and release APK both report `com.example.studexa`, version code `1`, version name `0.1.0`, min SDK `24`, target SDK `36`; only the installed APK reports `application-debuggable`.
  - `apksigner verify --print-certs` reports the same signer SHA-256 digest for both: `dd6497fcd859d1e15d67353978fc1dc5d51299bc0c183caa277ff2d6171d1e5a`. Their APK file sizes differ, so they are not the same build.
  - No install or uninstall was attempted. Date: 2026-09-20.
...
### 2026-09-20 - Connected-phone APK compatibility check
- Read `IMPLEMENTATION_LOG.md` and used ADB only for read-only inspection of the newly connected CPH1933 phone; no install, uninstall, data clear, or app launch was performed.
- The phone currently has a debuggable `com.example.studexa` APK installed (version code `1`, version name `0.1.0`, last updated 2026-09-19 07:08:15). The new `app-release.apk` has the same package ID and version but does not carry the `application-debuggable` flag; the APK files differ in size.
- Pulled a temporary comparison copy of the installed APK and verified both signatures. Both use Android Debug certificate SHA-256 `dd6497fcd859d1e15d67353978fc1dc5d51299bc0c183caa277ff2d6171d1e5a`, so Android should accept the new release APK as an in-place update while retaining app data. Installation itself remains untested.
- The Android device is now connected, superseding the earlier no-device blocker. The signed-in Office-app viewing and launcher checks remain pending.
- Files changed: `docs/IMPLEMENTATION_LOG.md` only. Next task remains the signed-in native DOCX/PPTX Office-app walkthrough.

### 2026-09-20 - APK location and installed-app status check
- Read this log, then checked the connected Android device and the current local release artifact without changing the phone or application code.
- ADB found CPH1933 (`584dac09`) with `com.example.studexa` version `0.1.0` installed. Package flags still include `DEBUGGABLE`, and `lastUpdateTime` remains 2026-09-19 07:08:15, so the release APK has not replaced the installed debug build on this connected phone.
- Verified the local file `build/app/outputs/flutter-apk/app-release.apk` exists (75,953,401 bytes; SHA-256 `8A842EB29F11ADEA64E3D1EBD13C873529E544E77562DAA93C565AC18BCAADE7`). The Android manifest label is `studexa`, which identifies the app in the phone's app list.
- No installation, uninstall, transfer, or source change was made. Only this log was updated. Any other phone's installation state remains unknown until that phone is connected or checked directly.

### 2026-09-20 - Shareable Studexa APK filename
- Read this log and confirmed the previously verified `app-release.apk` exists and no `Studexa.apk` already occupied the destination.
- Copied the release APK to `build/app/outputs/flutter-apk/Studexa.apk`, retaining the original Flutter output. Both files are 75,953,401 bytes and have identical SHA-256 `8A842EB29F11ADEA64E3D1EBD13C873529E544E77562DAA93C565AC18BCAADE7`.
- The filename change affects only the file used for sharing; app label, package ID, launcher icon, signing, version, and behavior remain unchanged. No phone installation was attempted.
- Files changed: new ignored build artifact `build/app/outputs/flutter-apk/Studexa.apk` and `docs/IMPLEMENTATION_LOG.md`. The signed-in native DOCX/PPTX Office-app walkthrough remains the next task.

### 2026-09-20 - Quiz repetition prevention and teacher question completion
- Read this log and the current code before implementing the agreed quiz-generation plan. The generator now prompts against every accepted fact, validates candidates, and filters same-fact repeats across batches and question types while retaining distinct facts. Updated clients opt in to private shortfall drafts; older clients keep exact-count behavior.
- Added saved draft metadata (`requestedQuestionCount`, `selectedQuestionTypes`, `extraGenerationAttempted`) and a one-attempt `Generate more` continuation that checks owner/class/material/type/draft status and merges fresh questions transactionally without discarding teacher additions. The teacher can use the partial draft or add a question of any supported type (Multiple Choice, True/False, Fill-in-the-Blank, Identification, Enumeration) before finalizing or publishing. Manual questions are marked `origin: manual`, validated, and saved with updated points.
- Verified 23/23 Node generator tests, 196/196 Flutter tests, clean `flutter analyze`, and clean `git diff --check`. Firebase emulator fixture passed 50 assertions including shortfall persistence, teacher-question retention, and refusal of a second continuation; the live Gemini emulator run passed 42 checks and saved Actual and Practice quizzes with 10 questions each.
- Built `build/app/outputs/flutter-apk/app-release.apk` and copied it to `build/app/outputs/flutter-apk/Studexa.apk` (76,264,773 bytes each, identical SHA-256 `07FCAAED421C788880491CD686F4310805D7BCAA5A1EAAC76E612C365EF14CA9`). Android `apksigner verify --verbose` passed with one v2 signer. No phone install was performed.
- Attempted to deploy `generateQuizHttp` and `generateQuiz` to `studexa-b5e55`. Firebase stopped before upload: Secret Manager HTTP 403 says billing must be enabled on that project. Thus the local/emulator backend is verified, but production still uses the prior function versions and the new partial/Generate more flow cannot work there yet.
- Files changed: `functions/index.js`, `functions/quiz_generator.js`, `functions/test/quiz_generator.test.js`, `functions/test/firebase_integration.js`, `lib/models/quiz_model.dart`, `lib/services/quiz_generation_client.dart`, `lib/services/quiz_service.dart`, `lib/screens/teacher/quiz_detail_screen.dart`, `lib/screens/teacher/upload_generate_quiz_screen.dart`, new `lib/screens/teacher/manual_question_dialog.dart`, `test/quiz_generation_client_test.dart`, `test/ui_navigation_walkthrough_test.dart`, new `test/manual_question_dialog_test.dart`, and this log. The two APKs are ignored build artifacts. Pre-existing `docs/command.md` was untouched.
- Next task: enable Firebase project billing, redeploy the two quiz-generation functions, and verify a real production shortfall/continuation before distributing the updated APK. The older signed-in DOCX/PPTX device walkthrough remains pending.

### 2026-09-20 - Supabase Edge Function quiz-generation migration (deployment pending)
- Read this log and the current plan before changing quiz generation. Preserved the existing uncommitted repetition-filtering, partial-draft, one-time Generate more, and manual-question work; the old Firebase quiz functions remain in place for already-installed APKs.
- Added `supabase/functions/generate-quiz` with Firebase ID-token verification through Firebase Auth, teacher/class/material authorization, and Firestore REST reads/writes under the same user token and existing Security Rules. The function ports the current Gemini prompt, grounded-question validator, repetition filter, and boilerplate exclusions. It saves at most one 10-question Gemini batch per invocation, uses deterministic generation IDs and batch IDs for retries, and uses Firestore update-time preconditions to merge concurrent manual edits.
- Updated the Flutter quiz client to coordinate initial and one-time Generate more batches for the existing 1-50 question selection, and pointed its default endpoint to the Supabase function. Firebase Auth, Firestore, Supabase Storage, quiz publication, attempts, scoring, and results were not migrated. No Flutter package, service-account key, or Firebase Blaze billing was added.
- Added `supabase/config.toml` with gateway JWT verification disabled only for `generate-quiz`; the handler explicitly verifies Firebase tokens before Gemini or Firestore access. Added `docs/SUPABASE_QUIZ_GENERATION_SETUP.md` with secret, login, deploy, and signed-in smoke-test steps.
- Verification: 12 Edge Function Node tests passed, including 1/10/30/50-question batches, student/invalid-token denial, Actual-to-Practice reference, initial and continuation retry IDs, concurrent teacher additions, Firestore update-time conflicts, and one-time Generate more. The ported Gemini validator passed a direct live Gemini one-question smoke test using the ignored local key. Deno type-check and production-source lint passed; a local Deno server returned HTTP 405 for GET and 401 for unauthenticated POST. Flutter analysis passed with no issues; the full Flutter suite passed 197/197 before the final network-retry refinement, and the focused client suite passed 17/17 afterward. The final `flutter build apk --release` succeeded; the APK's SHA-256 is `F8EB8516C12ABF4D67655312F07798149D2A7EB75B24E2B737DE85A49546668A`, and `apksigner` verified its v2 signature.
- Deployment and signed-in production verification remain pending: the Supabase CLI is available through `npx`, but `supabase projects list` reports no access token on this computer. The owner must add `GEMINI_API_KEY` as a Supabase Edge Function secret and run `npx supabase login` locally. Do not distribute this new APK until the Edge Function is deployed and a signed-in teacher test passes; the earlier `Studexa.apk` copy was not refreshed in this pass.

### 2026-09-20 - Supabase quiz-generation deployment and live verification
- The owner completed Supabase login and configured the `GEMINI_API_KEY` Edge Function secret. Supabase CLI 2.117.0 failed to reach the Management API through local HTTPS inspection; pinned CLI 2.102.0 with Node's system CA connected successfully. Deployed `generate-quiz` to project `zuidphgogdwyrtndbgkx` with `--use-api --no-verify-jwt`. Firebase token verification remains enforced inside the handler.
- A signed-in production smoke test exposed a Firestore rules interaction: direct reads of nonexistent quiz documents return HTTP 403 because quiz read rules depend on existing document fields. Changed initial draft lookup to a class-scoped Firestore query on `classId` and `generationSessionId`, which the teacher's existing rules authorize even when no quiz exists. Kept Firestore writes under the teacher's Firebase ID token and kept create conflicts/retry IDs idempotent.
- Redeployed the corrected function. Using temporary Firebase teacher and student accounts plus temporary Firestore class/material records, verified Actual generation returned one grounded question (HTTP 200), a repeated initial request returned the same quiz and question (HTTP 200), Practice generation using the Actual reference returned one question (HTTP 200), and student generation was refused (HTTP 403). Unauthenticated POST returned 401 and GET returned 405. All temporary quiz, material, class, profile, and Auth records were deleted successfully.
- The 13 focused Node Edge Function tests passed, covering 1/10/30/50-question batching, authorization, Actual/Practice flow, retries, concurrent manual edits, one-time Generate more, and the new Firestore query. Deno check and production-source lint passed. Linting the Node-only test fixtures with Deno still reports `require-await` findings in their mock functions; the tests themselves pass. The final Edge Function deployment succeeded after the test pass. The previously built Android release APK remains valid because this follow-up changed only Edge Function code and documentation; its v2 signature was verified in the preceding entry. Refreshed the ignored shareable `build/app/outputs/flutter-apk/Studexa.apk` from `app-release.apk`; both have SHA-256 `F8EB8516C12ABF4D67655312F07798149D2A7EB75B24E2B737DE85A49546668A`.
- Files changed in this follow-up: `supabase/functions/_shared/firestore_rest.mjs`, `supabase/functions/_shared/quiz_edge.mjs`, `supabase/functions/test/firestore_rest.test.mjs`, `supabase/functions/test/quiz_edge.test.mjs`, `docs/SUPABASE_QUIZ_GENERATION_SETUP.md`, this log, and the ignored `Studexa.apk` copy. No phone installation or in-app screen walkthrough was performed.

### 2026-09-20 - Install updated Studexa APK on connected phone
- Confirmed the CPH1933 phone (`584dac09`) was connected and the shareable `build/app/outputs/flutter-apk/Studexa.apk` was present. The APK reports package `com.example.studexa`, version `0.1.0`, and the same signing certificate SHA-256 (`dd6497fcd859d1e15d67353978fc1dc5d51299bc0c183caa277ff2d6171d1e5a`) recorded for the phone's earlier debug installation.
- Ran `adb -s 584dac09 install -r build/app/outputs/flutter-apk/Studexa.apk`; Android returned `Success`. No uninstall or app-data-clear command was used. The package retains its original `firstInstallTime` of 2026-09-09 21:11:43, has a new `lastUpdateTime` of 2026-09-20 21:44:01, and no longer has the `DEBUGGABLE` flag.
- Launched Studexa from its launcher activity. Android reports `com.example.studexa/.MainActivity` as the resumed foreground activity with a live app process. Signed-in quiz generation and the Office-app viewing walkthrough were not exercised on this phone during the install check.
- Files changed: this log only. The installed APK is the ignored release artifact already built and verified in the preceding migration entry.

### 2026-09-21 - Install Studexa APK on connected Android 16 phone
- Found one connected device, `25078RA3EA` (`BUVW7TKVJR5T6DBA`), running Android 16. The existing `com.example.studexa` installation was version `0.1.0`; the shareable `Studexa.apk` matched `app-release.apk` byte-for-byte by SHA-256 (`F8EB8516C12ABF4D67655312F07798149D2A7EB75B24E2B737DE85A49546668A`) and was signed with the project's recorded debug certificate.
- Installed the release APK over USB using `adb -s BUVW7TKVJR5T6DBA install -r build/app/outputs/flutter-apk/Studexa.apk`; ADB returned `Success`. No uninstall or data-clear command was used. The package's `firstInstallTime` remained 2026-09-21 09:37:12 and `lastUpdateTime` advanced to 2026-09-21 09:39:09.
- Launched the app; Android reported a live `com.example.studexa` process and `com.example.studexa/.MainActivity` as the top resumed activity. Signed-in quiz behavior was not retested during this installation.
- Updated the local `docs/command.md` USB example to work with any single connected Android phone instead of the previous phone's fixed serial. Files changed: `docs/command.md` and this log only; the installed APK was an existing ignored build artifact.

### 2026-09-23 - Firebase client API-key source hygiene
- Removed all literal Firebase API keys from `lib/firebase_options.dart`. Each platform now reads its key from a required `FIREBASE_*_API_KEY` compile-time dart-define and reports a clear configuration error when the selected platform's value is missing.
- Added tracked `firebase.config.example.json`, ignored populated `firebase.config.json`, and updated README run/build commands to use `--dart-define-from-file=firebase.config.json`. Re-running FlutterFire configuration can overwrite the customized options file, so its header now records the required integration.
- This change prevents accidental key exposure from the tracked Dart file but does not treat Firebase client keys as authorization secrets: compiled clients and the standard native Firebase configuration can expose them. Firebase Security Rules remain active; application/API restrictions and App Check must be configured and enforced in the Firebase/Google Cloud consoles for actual abuse protection.
- Verification: `flutter analyze` passed with no issues; `flutter build apk --debug --dart-define-from-file=firebase.config.json` built `build/app/outputs/flutter-apk/app-debug.apk`; `git check-ignore -v firebase.config.json` confirmed the populated file is ignored; `rg` found no `AIzaSy` literal in `lib/firebase_options.dart` or the tracked example. The build emitted only the existing future Kotlin Gradle Plugin compatibility warning.
- Files changed: `lib/firebase_options.dart`, `.gitignore`, new `firebase.config.example.json`, `README.md`, and this log. The ignored local `firebase.config.json` and debug APK are local-only. Next task remains the pending signed-in DOCX/PPTX Android walkthrough; Firebase App Check and console key restrictions should also be enabled and monitored before enforcing them for production traffic.

### 2026-09-23 - Per-Teacher Gemini API keys with Studexa backup
- Added Teacher-only **Teacher Account → Gemini API Settings**. Teachers can enter an obscured key, show/hide it while editing, use **Save and Verify**, replace it without losing the previous working key when validation fails, remove it without resetting allowances, and see only masked status such as `Configured ••••1234`. Student menus have no Gemini settings entry.
- Added the complete in-app Google AI Studio guide and Copy Link action for `https://aistudio.google.com/api-keys`, including teacher-owned project, free-tier/billing responsibility, key-sharing/revocation, Workspace eligibility, Usage, and Rate limits guidance. No URL-launching or other Flutter package was added.
- Added and deployed Supabase Edge Function `teacher-gemini-key` with Firebase ID-token verification and Firestore Teacher-role enforcement for GET/PUT/DELETE. Added the Vault migration `202609230001_teacher_gemini_credentials.sql`: full keys are encrypted in Supabase Vault; the private mapping stores Firebase UID, Vault secret ID, last four characters, validation status, and timestamps. Public RPC execution is revoked from `public`, `anon`, and `authenticated` and granted only to `service_role`.
- Updated and redeployed `generate-quiz`. A verified Teacher's personal key is primary. Teachers without one can reserve three lifetime successful Studexa-backed sessions. Personal-key quota, timeout, network, and temporary Gemini 5xx failures can use at most two Studexa backup sessions per UTC day; rejected/revoked/restricted keys do not fall back and must be replaced. Per-Teacher PostgreSQL advisory locks make concurrent reservations atomic. Actual, Practice, and Generate More each use one `clientSessionId`, so all bounded batches and retries count once; failed pre-save reservations are released and saved-request retries reconcile a pending reservation.
- Generation responses and quiz drafts now carry only non-secret usage metadata (`personal`/`backup`, reason, and remaining allowances). Flutter shows a pre-generation setup/temporary-use dialog and a top feedback banner after backup use. Structured setup, invalid-key, exhausted-allowance, backup-unavailable, and credential-service errors preserve partial drafts.
- Applied both tracked Supabase migrations and deployed `teacher-gemini-key` and `generate-quiz` to project `zuidphgogdwyrtndbgkx` with the Edge gateway JWT check disabled only because both handlers explicitly verify Firebase ID tokens. The existing server-side Supabase `GEMINI_API_KEY` remains the backup and is not included in the APK.
- Production verification used temporary Teacher and Student accounts. Confirmed unauthenticated key access returned 401, Student key access and Student generation returned 403, invalid-key validation returned 400 without echoing the key, Teacher status began with three temporary sessions, Actual and Practice each generated a usable one-question draft using backup reason `grace`, and status then reported one remaining session. No complete Gemini key appeared in responses or application log statements. All temporary Auth, profile, class, material, and quiz records were deleted.
- Verification passed: 22/22 Node Edge Function tests, including authorization, masked responses, credential isolation, validation, three-session exhaustion, batch idempotency, daily fallback exhaustion, invalid-key refusal without backup, backup unavailability, repetition prevention, partial drafts, and 1/10/30/50-question scenarios; `flutter analyze` reported no issues; the full Flutter suite passed 205/205, including Teacher-only settings, guide text, obscured entry, masking, grace prompts, blocked fourth session, structured errors, and unchanged Student navigation. The standalone `deno` executable is unavailable on this computer; successful Supabase CLI bundling/deployment of both functions served as the production Deno compatibility check.
- Built `build/app/outputs/flutter-apk/app-release.apk` with `--dart-define-from-file=firebase.config.json` and refreshed the ignored shareable `Studexa.apk`. Both are 76,298,205 bytes with SHA-256 `32FA2403FF7A692A08EE454F7A6E91F5D5C4771F01B355E30CF2F95AA5B59902`; Android `apksigner` verified one valid v2 signer. No phone installation was performed in this pass.
- Attempted the approved retirement of only Firebase `generateQuiz` and `generateQuizHttp` after Supabase production verification. Google Cloud rejected both deletions with HTTP 403: write access is disabled until the Firebase project has a billing account. Both legacy endpoints therefore remain deployed; `extractText` was untouched. This preserves old APK behavior but means endpoint retirement is the only incomplete rollout step and cannot be completed under the current no-Blaze decision.
- Main files added: `lib/screens/teacher/gemini_api_settings_screen.dart`, `lib/services/teacher_gemini_key_service.dart`, `lib/widgets/gemini_api_generation_guard.dart`, `supabase/functions/teacher-gemini-key/index.ts`, `supabase/functions/_shared/firebase_identity.mjs`, `supabase/functions/_shared/teacher_credentials.mjs`, `supabase/functions/_shared/teacher_gemini_key_edge.mjs`, `supabase/migrations/202609230001_teacher_gemini_credentials.sql`, and their focused Node/Flutter tests. Updated generation, quiz metadata, Teacher menu, Supabase config, integration endpoint, responsive menu tests, and this log. Existing Firebase Auth, Firestore schema/rules, Supabase Storage, publication, attempts, scoring, and results remain unchanged.
