...
async function generateSingleBatch(request, {
  apiKey, candidateModels, fetchImpl, timeoutMs, rejectQuestion, avoidStems,
}) {
  const { extractedText, questionTypes, questionCount, isActual, sourceQuizContext } = request;
  const avoidClause = avoidStems && avoidStems.length
    ? `\nDo not duplicate or rephrase any of these already covered questions: ${avoidStems.slice(-10).join("; ")}.`
    : "";
...
async function generateQuizQuestions(request, {
  ...
  try {
    ...
    for (let round = 0; round < maxRounds; round++) {
      ...
      try {
        const batchResult = await generateSingleBatch(batchRequest, {
          ...
          avoidStems: Array.from(seenStems),
        });
        ...
        if (round >= maxRounds - 1 && collectedQuestions.length < questionCount) throw err;
        continue;
      }
      ...
      for (const rawQ of batchRaw) {
        ...
        try {
          ...
          const stem = normalize(validated.question).replace(/[^a-z0-9 ]/g, "");
    ...
    if (collectedQuestions.length < questionCount || questionTypes.some((t) => !seenTypes.has(t))) {
